function classifyotherscript(flag, classnum, randseed, allflag, maximgperfold, sdaflag, weightflag, cidx, gidx, nbrs, classifierid, otherflag)
%CLASSIFYOTHERSCRIPT uses classifier (with flag) to train image with otherflag, only used for ambiguous or punctate_composite class
%   CLASSIFYOTHERSCRIPT(FLAG, CLASSNUM, RANDSEED, ALLFLAG, MAXIMGPERFOLD, IDXFOLD, SDAFLAG, WEIGHTING, CIDX, GIDX, NBRS, CLASSIFIERID, OTHERFLAG)
%
% Input:
%   FLAG specifiy which classifier to use
...
%   OTHERFLAG specifiy which feature matrix to use (either ambiguous or punctate_composite)
%					       
% Output:					       
%    savefilestr = sprintf('celllevel/result/COMBINE_F%dC%dN%dA%dM%dR%d_S%dW%d_C%dG%dN%d%s.mat', ...^M
%	      otherflag, classnum, nfold, allflag, maximgperfold, randseed, sdaflag, weightflag, cidx, gidx, nbrs, classifiername);^M
%
%   See also classify_other
%
