...
#ifndef lint
  static char version[] = "@(#) Trist Module 2.5";
...
static int hull_compare();

#ifdef __DEBUG__
 static void count_edges(), kill_edge_dope();
 static int edge_index(), *new_edge_dope();
#endif
 
static int (* permute_hook) () = NULL;  /* Called at end of trist_permute () */
     ...
     /* Copies Trist a to Trist b, assuming b empty and large enough. */
{
  ...
  bcopy (a->triangle, b->triangle, 
         (a->last_triangle + 1) * sizeof (Trist_record));
  bcopy (a->data, b->data, 
...
void trist_hull_facet_set (e, flag)
     int e;
     int flag;
...
static void equality (number, left, right)
              ...
              of the structure as read-only. */
{
  ...
  int f_size = sizeof (Trist_record) + st->data_size;
  if (not st)
  ...
  ti.bpt      = sizeof (Trist_record);
  ti.bytes    = (st->last_triangle + 1) * f_size;
  ti.maxbytes = (st-> max_triangle + 1) * f_size;
  return (&ti);
}
...
void trist_hull_copy (vertex, vertex_hull)
     int vertex[];
     int vertex_hull[]; /* output */
...
void trist_data_size (record_size)
     ...
     /* Allows the user to maintain a "data" record (w/ record_size bytes)
        ...
        USAGE: >
               ...
               > :
               > trist_set (...);
               > trist_data_size (sizeof (WHATEVER));
               >  :
...
void trist_nvx (a, b, c, nx_ptr, ny_ptr, nz_ptr, s_ptr)
     int a, b, c;
     Lia_ptr *nx_ptr, *ny_ptr, *nz_ptr, *s_ptr;  /* output */
...
static void count_edges (edges, edges_hull)
     int *edges, *edges_hull;  /* output */
     /* trist_num() refinement */
{
  int i, j, ei, i1, i2;
  int e, eh;
  char *aux;     /* used as bit vector to count edges! */
  int *dope;     /* we also need some dope, man! */
  int max_index;
  ;
  dope = new_edge_dope (&max_index);
  aux = MALLOC (char, (max_index div8) + 1);
  BZERO (aux,   char, (max_index div8) + 1);
  ;
  eh = 0;
  trist_for (j)
...
static int *new_edge_dope (max_index)
     int *max_index;
{
  int i, m = st->max_org;
  int *dope;
...
static void kill_edge_dope (dope)
     int *dope;  /* input/output */
{
  FREE (dope);
}
