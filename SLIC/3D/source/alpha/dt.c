...
extern trist__internal_magic_number;
...
static int (* positive3_hook) () = NULL;
static int (* in_sphere_hook) () = NULL;

/*--------------------------------------------------------------------------*/

/* local procedures and variables */

static void test_complex(), test_hull(), test_hull_recursive();
static void test_triangulation(), test_tetra_global();

static int is_command();
static void int_param_push2();
static void ffp_param_push2();
...
void dt_save (path_name, type, s, q)
     char path_name[];
     int type;
     Trist *s;
     Trist_num *q;
  ...
  { /* using binary format */
    ...
    if (s->magic != MAGIC)
      basic_error ("dt_load: wrong magic number (%d != %d)\n",
                   s->magic, MAGIC);
    if (dt->bpt != sizeof (Trist_record))
      basic_error ("dt_load: wrong Trist record size (%d != %d)\n",
                   dt->bpt, sizeof (Trist_record));
...
Dt_input_scan *dt_input_scan (data_path)
     char data_path[];
...
static int is_command (is, str, command, nchars)
     Dt_input_scan *is;
     char *str, *command;
     int nchars;
...
void dt_input_load (data_path, data)
     char data_path[];
     Dt_input_scan *data;  /* input/output */
        ...
        three normal, and one which is sum of squares (+ weight, if any)! */
{
      ...
      if (basic_strip (r) and (r[0] != '#'))
        {
          ...
          if (data->fix_w)
            ...
            io = sscanf (r, "%ld %ld %ld %ld", &x, &y, &z, &w);
          switch (io)
            {
             case 3:
              { 
  ...
  basic_fclose (inp);
  Assert_always (i == data->n);
}

static void int_param_push2 (i, j, x)
     int i, j;
     int x;
...
static void  ffp_param_push2 (i, j, x, w, a)
     int i, j;
     double x;
     int w, a;
     /* dt_input_load() refinement */
...
void dt_test_hooks (positive3, in_sphere)
     int (* positive3) ();
     int (* in_sphere) ();
...
static void test_hull_recursive (hb, h)
     Basic_byte hb[];
     int h;
{
  int t = TrIndex (h);
...
int dt_test_open_tetra (a, x, y, z, o)
     int a, x, y, z, o;
...
static void test_tetra_global (a, b, c, d, vertex, m,
                               closest_flag, delaunay_flag)
     int a, b, c, d;
     int vertex[];
     int m;
     int closest_flag, delaunay_flag;
...
int dt_test_pedantic (vertex, m, closest_flag)
     int vertex[];
     int m;
     int closest_flag;
...
void dt_print_info_sec (info, secs, title)
     FILE *info;
     double secs;
     char title[];
{
