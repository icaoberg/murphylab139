function [procimage,imagemask,resimg] = ...
    ml_preprocess_scale( image, cropimage, way, bgsub,threshmeth, scale)
% [PROCIMAGE,IMAGEMASK,RESIMG] = ML_PREPROCESS_SCALE( IMAGE, CROPIMAGE, WAY, BGSUB, SCALE)
%
...
%%% Theshold
if (max(max(image))>0)
    imagemask = ml_threshcrop_scale(image, threshcropimage, threshmeth, scale);
    if (max(max(imagemask))>0)
