function mask = ml_threshcrop_scale(image, cropimage, method, scale)
% ML_THRESHCROP_SCALE generates a cropped, thresholded, cleaned  image
% ML_THRESHCROP_SCALE(IMAGE, CROPIMAGE, SCALE), where image is the IMAGE 
...
% SCALE is a scale (double) used to modify the threshold of nih/rc thresholding
%
% 06 Aug 98
...
% $Id: ml_threshcrop.m,v 1.4 2005/11/05 20:06:51 tingz Exp $
...
case 'nih'
    ...
    Ithresh = im2bw(Iscaled, Timage*scale) ;
case 'rc'
    Iscaled = uint8(floor(ml_rcscale(image))) ;
    Timage = ml_rcthreshold(Iscaled);
    Ithresh = im2bw(Iscaled, Timage*scale/255) ;
end
